...
namespace Blamite.Plugins
{
	public class AscensionPluginWriter : IPluginVisitor
	{
		...
		public void VisitAscii(string name, uint offset, bool visible, int length, uint pluginLine, string tooltip,string tra)
		{
		...
		public void VisitUtf16(string name, uint offset, bool visible, int length, uint pluginLine, string tooltip,string tra)
		{
			// TODO: does Ascension support this?
		}

		public void VisitHexString(string name, uint offset, bool visible, int length, uint pluginLine, string tooltip, string tra)
		{
			WriteValueStart("bytearray", name, offset, visible);
			_output.WriteAttributeString("length", length.ToString(CultureInfo.InvariantCulture));
			_output.WriteEndElement();
